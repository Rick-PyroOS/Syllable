...
CalendarView::CalendarView(const os::Rect& cRect,const os::String& cName,os::Messenger* parent,const os::DateTime& date, bool bShowDateView,uint32 nResizeMask, uint32 nFlags) : View(cRect,cName,nResizeMask,nFlags)
{
	m = new Private();
	m->m_cDate = date;
...
{
	DrawFrame(cRect,os::FRAME_RAISED);
}
...
void CalendarView::_Layout()
{
	...
	cRect.Resize(4,4,-4,-4);
	
	m->m_pcLayoutView = new  os::LayoutView(cRect,os::String().Format("%s_layoutivew",GetName().c_str()).c_str(),NULL,GetResizeMask(),GetFlags());
